...
3. [Convolutional Layer: The Feature Extractor](#3-convolutional-layer-the-feature-extractor)
4. [Pooling Layer: Downsampling](#4-pooling-layer-downsampling)
5. [Forwardpropagation: Math of Feature Maps](#5-forwardpropagation-math-of-feature-maps)
6. [Backpropagation: Training the Convolutional Layer](#6-backpropagation-training-the-convolutional-layer)
...
A typical CNN architecture consists of a sequence of layers followed by a classifier. Unlike standard ANNs where neurons are fully connected, CNN layers are designed to exploit the **spatial structure** of data like images. 
...
## 3. Convolutional Layer: The Feature Extractor
...
| **Filter Size** | $F$ | Dimensions of the kernel (e.g., $3$). |
...
## 6. Backpropagation: Training the Convolutional Layer

Training CNNs relies on **Gradient Descent** guided by **Backpropagation** using the **Chain Rule**. The process must account for **weight sharing** and **pooling**.
...
* **Max Pooling:** The gradient is passed only to the neuron position that held the maximum value in the forward pass. This is done using a stored "mask" from the forward pass.
* **Average Pooling:** The gradient is distributed equally among all neurons in the pooling window.
...
* This is mathematically a **convolution** between the previous layer's input ($\mathbf{A}^{[l-1]}$) and the error sensitivity ($\delta^{[l]}$).
...
$$\frac{\partial \mathcal{L}}{\partial \mathbf{A}^{[l-1]}} = \text{Full Convolution}\left( \delta^{[l]}, \text{Flipped } \mathbf{W}^{[l]} \right)$$
...
### 7.1 Flattening
The output of the final $\text{CONV}$ or $\text{POOL}$ layer (e.g., $4 \times 4 \times 128$) is converted into a $1$D feature vector.
...
* **Input:** The flattened vector containing the high-level features learned by the CNN layers.
* **Output:** The final $\text{FC}$ layer usually uses a **Softmax** activation to output the probability distribution over all classes.

### 7.3 Backpropagation in FC Layers
Standard ANN backpropagation rules (as defined for a regular ANN) apply directly to these layers.